...
class TARLPostPoolTrainer(LightningModule):
    ...
    def forward(self, x_t:ME.SparseTensor, s_t, x_tn:ME.SparseTensor, s_tn):
        ...
        with torch.no_grad():
            ...
            h_tn = self.model_k(x_tn)
            # list segments and the mean over its point-wise features
            (h_tnc, h_tnf) = list_segments_points(h_tn.C, h_tn.F, s_tn, self.hparams['train']['sample_points'], avg_feats=False)
            
            h_tnc, h_tnf, pad_t_masks = pad_batch(h_tnc, h_tnf)

            # projection head over the segment-wise mean representation
            z_tn = self.proj_head_k(h_tnf)
            z_tn = nn.functional.normalize(z_tn, dim=-1)

            pad_t = (~pad_t_masks).float().unsqueeze(-1) # (Ns, 300, 1)
            z_tn_sum = (z_tn * pad_t).sum(dim=1)   # (Ns, d)
            valid_t = pad_t.sum(dim=1)  # Shape: (Ns, 1)
            # Avoid division by zero
            valid_t = valid_t.clamp(min=1)
            z_tn = (z_tn_sum / valid_t).unsqueeze(0)  # Shape: (Ns, d)
...
class OneWayTARLTrainer(LightningModule):
    def __init__(self, cfg: dict, data_module: LightningDataModule = None):
        super().__init__()
        # name you hyperparameter hparams, then it will be saved automagically.
        self.save_hyperparameters(cfg)
    ...
    def save_backbone(self, name='dino'):
        state = {
            'model': self.model_q.state_dict(),
            'epoch': self.current_epoch,
            'params': self.hparams,
